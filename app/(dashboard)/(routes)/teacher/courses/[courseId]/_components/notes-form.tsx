...
export const NotesForm = ({ initialData, courseId }: NotesFormProps) => {
  ...
  return (
    <div className="mt-6 border bg-slate-100 rounded-md p-4">
      ...
      {!isEditing && (
        <div
          className={cn("text-sm mt-2", !initialData.notes && "text-slate-500")}
        >
          {initialData.notes && <Preview value={initialData.notes} />}
        </div>
      )}
      {isEditing && (
        <Form {...form}>
      ...
      <div className="flex item-center gap-1 text-xs text-sky-700 mt-4">
        ...
        <p>Add sleeking notes that may help other teachers.</p>
      </div>
    </div>
  );
};
